        ...
        <div class="content-wrapper">
            ...
            <div class="profile-card">
                ...
                <div class="profile-header">
                    ...
                    <div class="profile-name">{{ $selectedNama }}</div>
                </div>
                <div class="profile-details">
                    <div class="detail-row">
                        <span class="detail-label">Total Kunjungan</span>
                        <span class="detail-value">{{ $detailRiwayat->count() }}x</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Kunjungan Terakhir</span>
                        <span class="detail-value">
                            {{ $detailRiwayat->first() ? \Carbon\Carbon::parse($detailRiwayat->first()->tanggal_kunjungan)->format('d M Y') : '-' }}
                        </span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Poli Terakhir</span>
                        <span class="detail-value">{{ optional($detailRiwayat->first())->poli ?? '-' }}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Dokter Terakhir</span>
                        <span class="detail-value">{{ optional($detailRiwayat->first())->dokter ?? '-' }}</span>
                    </div>
                </div>
                @else
                ...
                <div class="pasien-list">
                    ...
                    <a href="/riwayatpasien?pasien={{ urlencode($p->nama_pasien) }}{{ $search ? '&search='.urlencode($search) : '' }}"
                       class="pasien-item {{ $selectedNama === $p->nama_pasien ? 'selected' : '' }}">
                        <i class="fas fa-user-circle" style="margin-right:6px;"></i>{{ $p->nama_pasien }}
                    </a>
                    @endforeach
                </div>
                @endif
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="8" style="text-align:center;padding:20px;color:#999;">
                            {{ $selectedNama ? 'Belum ada riwayat untuk pasien ini' : 'Pilih pasien dari daftar kiri' }}
                        </td></tr>
                        @endforelse
                    </tbody>
                </table>
