...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-wrapper">
            ...
            <div class="queue-info">
                ...
                @if($currentQueue)
                    <div class="queue-number">{{ str_pad($currentQueue->no_antrean, 3, '0', STR_PAD_LEFT) }}</div>
                    <div class="patient-name">{{ $currentQueue->nama_pasien }}</div>
                    <div class="patient-detail">{{ $currentQueue->poli }} — {{ $currentQueue->dokter ?? '-' }}</div>
                    <!-- Tombol Selesai (hanya untuk yang sedang diperiksa) -->
                    <form method="POST" action="{{ route('antrean.selesai', $currentQueue) }}">
                        @csrf
                ...
                <form method="POST" action="{{ route('antrean.panggil') }}" style="margin-top:10px;">
                    @csrf
                    <button type="submit" class="btn-panggil">
                        <i class="fas fa-bullhorn"></i>
                        {{ $currentQueue ? 'Panggil Berikutnya' : 'Panggil Pertama' }}
                    </button>
                </form>
                @if($nextQueue)
                <div style="margin-top:12px;font-size:13px;color:#666;">
                    Berikutnya: <strong>{{ $nextQueue->nama_pasien }}</strong> (#{{ str_pad($nextQueue->no_antrean,3,'0',STR_PAD_LEFT) }})
                </div>
                @endif
            </div>
            ...
            <div class="queue-table-container">
                <table>
                    ...
                    <tbody>
                        @forelse($antreens as $antrean)
                        <tr>
                            <td>{{ str_pad($antrean->no_antrean, 3, '0', STR_PAD_LEFT) }}</td>
                            <td>{{ $antrean->nama_pasien }}</td>
                            <td>{{ $antrean->poli }}</td>
                            <td>{{ $antrean->dokter ?? '-' }}</td>
                            <td>
                            ...
                            <td>
                                <div style="display:flex;gap:5px;">
                                    <button class="btn-play"
                                        onclick="bukaModalEdit({{ $antrean->id }},'{{ addslashes($antrean->nama_pasien) }}','{{ addslashes($antrean->poli) }}','{{ addslashes($antrean->dokter ?? '') }}','{{ $antrean->status }}')">
                                        <i class="fas fa-pen"></i>
                                    </button>
                                    @if($antrean->status === 'diperiksa')
                                    <form method="POST" action="{{ route('antrean.selesai', $antrean) }}" style="display:inline;">
                                        @csrf
                                    ...
                                    @endif
                                    <form method="POST" action="{{ route('antrean.destroy', $antrean) }}" style="display:inline;" onsubmit="return confirm('Yakin hapus?')">
                                        @csrf @method('DELETE')
    ...
    <div class="modal-overlay" id="modalAntrean">
        <div class="modal">
            ...
            <form id="formAntrean" method="POST" action="{{ route('antrean.store') }}">
                @csrf
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="dokter" id="inputDokter">
                            ...
                            <option value="{{ $j->nama_dokter }}">{{ $j->nama_dokter }} ({{ $j->spesialis }})</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div id="noAntreanInfo" style="font-size:13px;color:#666;margin-top:5px;">
                    Nomor antrean akan ditetapkan otomatis: <strong>#{{ str_pad($nextNo, 3, '0', STR_PAD_LEFT) }}</strong>
                </div>
    ...
    <script>
        function bukaModalTambah() {
            ...
            document.getElementById('formAntrean').action = '{{ route("antrean.store") }}';
